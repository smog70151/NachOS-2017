...
int
Interrupt::WriteFile(char *buffer, int bufferLen, OpenFileID fid)
{
	return kernel->WriteFile(buffer, bufferLen, fid);
}

int
Interrupt::ReadFile(char* buffer, int size, OpenFileID fid)
{
	return kernel->ReadFile(buffer, size, fid);
}
