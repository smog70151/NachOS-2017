...
class FileSystem {
  public:
	...
	int Write(char* buffer, int bufferLen, int fid) {
		if (0 <= fid && fid < 20 ) 
			return fileDescriptorTable[fid]->Write(buffer, bufferLen); // Succeed
		else
			return -1; // Fail
	...
	int Read(char* buffer, int size, int fid) {
		if( 0 <= fid && fid < 20 ) 
			return fileDescriptorTable[fid]->Read(buffer, size);
		else
			return -1;
	}
