...
int Kernel::Exec(char* name)
{
	...
	//	cout << "Hello Execution : " << threadNum << endl;

	return threadNum-1;
	/*
    cout << "Total threads number is " << execfileNum << endl;
    for (int n=1;n<=execfileNum;n++) {
		t[n] = new Thread(execfile[n], n);
...
int Kernel::WriteFile(char* buffer, int bufferLen, int fid)
{
	return fileSystem->Write(buffer, bufferLen, fid);
}

int Kernel::ReadFile(char* buffer, int size, int fid)
{
	return fileSystem->Read(buffer, size, fid);
}

OpenFileID Kernel::OpenFile(char* fileName)
{
	return (OpenFileID)fileSystem->OpenFID(fileName);
}

int Kernel::CloseFile(OpenFileID fid)
{
	return fileSystem->Close(fid);
}
